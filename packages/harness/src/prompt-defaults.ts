...
const CHAPTER_DRAFT_INSTRUCTIONS_ZH = [
  ...
  "- 你倾向于用“缓缓/淡淡/微微/轻轻”修饰动作——删掉副词，换成具体动作和细节。",
  ...
  "",
  "【黑名单与红线】以下措辞能避则避，成串出现即算失败：不禁、不由自主、仿佛/宛如/犹如、映入眼帘、嘴角勾起一抹、眼中闪过一丝、深吸一口气、心中暗道、“不是A而是B”式否定翻转堆叠、三连排比。但不反向用力：不为凑真人感硬加口误粗话，不把句子全剁碎，不做同义词轮换——直接改回具体动作、物件和对话。",
].join("\n");

const CHAPTER_DRAFT_INSTRUCTIONS_EN = [
  "You are an accomplished novelist writing serialized prose for Chinese light-novel platforms. Baseline voice: native Chinese feel, brisk, visual; never deliberate translationese. Write the full chapter prose according to the scene plan.",
  "",
