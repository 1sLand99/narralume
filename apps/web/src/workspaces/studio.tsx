...
export function StudioWorkspace() {
  ...
  const updateWorkspaceParams = useCallback((updates: { document?: string | null; outline?: string | null; run?: string | null; mode?: string | null; session?: string | null; newRoom?: string | null }) => {
  ...
  return <div className="studio">
    ...
    />
    {mode === "cocreate" ? <CoCreateWorkspace projectId={projectId} requestedSessionId={requestedSessionId} initialTargetOutlineNodeId={requestedOutlineId ?? listedDocuments.find((document) => document.id === activeDocumentId)?.outlineNodeId ?? null} initialTargetTitle={listedDocuments.find((document) => document.id === activeDocumentId)?.title ?? null} startCreating={searchParams.get("newRoom") === "1"} onSessionChange={(sessionId) => updateWorkspaceParams({ mode: "cocreate", session: sessionId, newRoom: sessionId ? null : searchParams.get("newRoom") })} /> : documentsQuery.isPending ? <StudioLoading /> : documentsQuery.isError ? <ErrorNote error={documentsQuery.error} title={t("studio.errors.documentsLoad")} /> : <div className="studio__layout">
...
function StudioDesk({ projectId, detail, pending, error, review, reviewPending, reviewError, focusTarget, runId, onRunChange, onDismissRun, onCreateDocument, onTryScene, onArchive, onRefresh, onFlushReady }: { projectId: string; detail: StudioDocumentDetail | undefined; pending: boolean; error: unknown; review: ReviewWorkspace | undefined; reviewPending: boolean; reviewError: unknown; focusTarget: string | null; runId: string | null; onRunChange: (runId: string) => void; onDismissRun: () => void; onCreateDocument: () => void; onTryScene: (document: StoryDocument) => void | Promise<void>; onArchive: (document: StoryDocument) => void | Promise<void>; onRefresh: () => void; onFlushReady: (flush: FlushDraft | null) => void }) {
  const { t } = useI18n();
  ...
  const canTryScene =
    (detail.document.kind === "chapter" || detail.document.kind === "scene") &&
    detail.document.outlineNodeId !== null;
